...
/* TODO print as string */
	...
	uint32_t check_dalign_flag = 0x0100;

	if( ( (uint32_t) superblock->feature_flags & ~( supported_feature_flags ) ) != 0 )
	{
		if( ( (uint32_t) superblock->feature_flags & ~( check_dalign_flag ) ) != 0 ) {

			fprintf(
			stdout,
			"unsupported DALIGN in features flags\n");

			libcerror_error_set(
			error,
			LIBCERROR_ERROR_DOMAIN_RUNTIME,
			LIBCERROR_RUNTIME_ERROR_UNSUPPORTED_VALUE,
			"%s: unsupported DALIGN in features flags: 0x%04" PRIx16 ".",
			function,
			superblock->feature_flags );
